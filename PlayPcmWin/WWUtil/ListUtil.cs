...
namespace WWUtil {
    public class ListUtil {
        ...
        /// 一つもないときは要素数0個の配列が戻る。
        /// </summary>
        public static T[] GetArrayFragment<T>(List<T[]> from, long startPos, int count) {
            if (startPos < 0) {
        ...
        public static LargeArray<T> GetLargeArrayFragment<T>(List<T[]> from, long startPos, long count) {
            if (startPos < 0) {
                throw new ArgumentOutOfRangeException("startPos");
            }
            if (count < 0) {
